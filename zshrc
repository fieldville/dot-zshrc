...
plugins=(brew bundler compleat gem git git-extras git-flow git-remote-branch github heroku history node osx rails rake ruby rbenv svn vi-mode perl vagrant cap ssh-agent mvn sublime history-substring-search autojump gradle)
...
uname -a | grep -qi darwin && [[ -z "$TMUX" && ! -z "$PS1" ]] && tmux
